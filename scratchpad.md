...
- `esc` jump back is changing the selected window in the sidebar region, but doesn't change the terminal window in the terminal pane.
...
- As I type a really long terminal window name it does not wrap until I hit enter. It should wrap as I type.
- Think about how to auto-name claude windows.
- Show one symbol when a command is actively running, and a background animation when something is currently or has just recently been animating in the terminal. This way I can see at a glance which windows are active, and which ones have recently been active.
- Unless a window has been explicitly named, it should show the file location and currently running command. We need a way to differentiate multiple with the same name.
