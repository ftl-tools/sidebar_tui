...
**Ordinary `sb` does not use tmux as its backend.** The explicit [`sb tmux` preview](./tmux_preview) offers a native-target chooser plus read-only inspection; the hierarchy and limits below describe ordinary legacy `sb`. Sidebar still owns its PTYs and terminal emulation. There is currently one terminal pane per window, no pane splitting or linking windows between sessions, and window names remain globally unique. Saved metadata/scrollback restoration is Sidebar functionality, not a claim that tmux preserves processes across reboot.
