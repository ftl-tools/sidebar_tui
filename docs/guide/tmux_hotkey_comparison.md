...
- Sidebar **workspace** = tmux **session**.
- Sidebar **terminal/session** = tmux **window** with one terminal process.
- Creating a Sidebar terminal = creating a window, not splitting a pane.
- tmux panes have no Sidebar equivalent in this proposal.
...
- Bare command letters work only while the sidebar is focused. For example, `c` starts terminal creation and `s` opens the workspace chooser.
...
- Workspace actions `C`, `R`, and `K` target the current workspace, or the highlighted workspace when its chooser is open. In that chooser, `↑`/`↓`, `j`/`k`, and `Enter` navigate/select; `Esc`/`q` closes it back to the sidebar. Other window commands are disabled there.
...
| List all windows across workspaces | No direct cross-workspace list | No new aggregate view proposed; `s` selects workspace, then browse its windows | `tmux list-windows -a` | Same |
...
| Move window to workspace | `m`, choose workspace, `Enter` | `m`, choose workspace, `Enter` | `tmux move-window`; no default direct binding | Same |
...
| Open workspace chooser | `w` or `Ctrl+W` | `s`; choose with arrows or `j`/`k`, then `Enter` | `P → s` | `P → s` |
| Create workspace | Workspace overlay: `n` | `C`, name, then `Enter` | `tmux new-session`; no default prefix binding | `P → C` |
| Previous/next workspace | Choose in workspace overlay | `P`/`N`; direct `Alt+Up`/`Alt+Down` | `P → (`/`)` | Direct `Alt+Up`/`Alt+Down`; `P → P`/`N` |
| Rename workspace | Workspace overlay: `r` | `R` (also `$`), edit name, then `Enter` | `P → $` | `P → R`; dollar alias retained |
| Delete workspace | Workspace overlay: `d`, confirm | `K`, then confirm with workspace name and affected terminals shown | `tmux kill-session` | `P → K` immediately |
...
| `h`, `l`, arrows | Vi tree collapse/expand versus Omarchy `h` split; stock prefix arrows select panes and `l` selects last window | Sidebar is a flat window list scoped to a workspace. Up/down browse; `l` means last window. No collapse/expand or pane commands. A future hierarchical tree needs a separate key design. |
...
| `r`, `R`, `s`, `w` | tmux's window tree has its own refresh/sort/view commands, distinct from prefix commands | Give prefix semantics priority: rename window/workspace, choose workspace/window list. Do not claim complete tmux tree-key compatibility. |
...
| `x`, `K`, `Alt+Esc` | Omarchy directly kills panes/sessions without confirmation | No pane shortcuts in Sidebar; workspace `K` always confirms. Current Omarchy explicitly overrides `x` with `kill-pane`, removing stock confirmation. |
...
| Switch workspace | Toggle, `s`, navigate, `Enter`; browse its windows |
| Create workspace | Toggle, `C`, name, `Enter` |
...
This document proposes bindings and interaction semantics, not a tmux backend migration. Direct numbered switching, reordering, help, and the sticky command-mode routing need implementation and tests. No new panes, copy mode, multi-selection, or cross-workspace aggregate tree are included.
