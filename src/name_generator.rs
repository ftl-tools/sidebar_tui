//! Random window name generator using three 3-4 letter words.
//!
//! Generates memorable window names like "Fox blue moon" by combining
//! three random short words. The first word is capitalized.

use rand::seq::SliceRandom;
use rand::Rng;

/// List of common 3-4 letter words suitable for window names.
...
/// Generate a random window name from three 3-4 letter words.
/// The first letter of the first word is capitalized, rest are lowercase.
/// Words are separated by spaces.
///
/// # Example
/// ```
/// use sidebar_tui::name_generator::generate_window_name;
/// let name = generate_window_name();
/// // e.g., "Fox blue moon"
/// ```
pub fn generate_window_name() -> String {
    let mut rng = rand::thread_rng();
    generate_window_name_with_rng(&mut rng)
}

/// Generate a window name using a specific RNG (for testing).
pub fn generate_window_name_with_rng<R: Rng>(rng: &mut R) -> String {
...
/// Check if a name already exists in a list of existing window names.
...
/// Generate a unique window name that doesn't conflict with existing windows.
/// Tries multiple times to find a unique name before giving up and appending a number.
pub fn generate_unique_window_name(existing_names: &[&str]) -> String {
    let mut rng = rand::thread_rng();

    // Try up to 10 times to generate a unique name
    for _ in 0..10 {
        let name = generate_window_name_with_rng(&mut rng);
    ...
    let base_name = generate_window_name_with_rng(&mut rng);
...
mod tests {
    ...
    #[test]
    fn test_generate_window_name_format() {
        let name = generate_window_name();
    ...
    #[test]
    fn test_generate_window_name_deterministic_with_seed() {
        let mut rng1 = StdRng::seed_from_u64(12345);
        let mut rng2 = StdRng::seed_from_u64(12345);

        let name1 = generate_window_name_with_rng(&mut rng1);
        let name2 = generate_window_name_with_rng(&mut rng2);

        assert_eq!(name1, name2, "Same seed should produce same name");
    }

    #[test]
    fn test_generate_window_name_variability() {
        // Generate many names and check they're not all the same
        let names: Vec<String> = (0..10).map(|_| generate_window_name()).collect();
    ...
    #[test]
    fn test_generate_unique_window_name_avoids_existing() {
        let existing = vec!["test1", "test2"];
        let name = generate_unique_window_name(&existing);
